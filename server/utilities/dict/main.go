...
func main() {
	...
	Init()
	filename := "/Users/xiongda/GolandProjects/zhaofang/server/utilities/dict/output.txt"
...
func processLine(line string, lineNumber int) {
	if strings.Contains(line, "encryptHouseName") {
		...
		//err := global.GVA_DB.Exec("insert into dict_house (house_open_id, encrypt_house_name, unit_open_id) values (?, ?, ?)", houseId, houseName, unitId).Error
		//if err != nil {
